...
public class MessageResourceService {
	...
	public MessageResource getMensajeDesc(String parametro, String catRoot) {
		// Retrieve session from Hibernate
		Session session = sessionFactory.getCurrentSession();
		// Create a Hibernate query (HQL)
		Query query = session.createQuery("FROM MessageResource mr where mr.catRoot =:catRoot and ((lower(mr.catKey) =:parametro or lower(mr.spanish) =:parametro or lower(mr.english) =:parametro))");
		query.setParameter("parametro", parametro.toLowerCase());
		query.setParameter("catRoot",catRoot);
